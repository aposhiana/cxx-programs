...
PiComputer::PiComputer(unsigned int numDigits) : m_numDigits(numDigits)
{
    // Use i to be the digit to compute, therefore start at 1
    for (unsigned int i = 1; i <= m_numDigits; i++) {
        m_taskQueue.push(i);
    }
}

bool PiComputer::computePiDigitWithFeedback() {
    unsigned int digitNum;
    bool taskAvailable = m_taskQueue.pop(digitNum);
    if (!taskAvailable) {
        return false;
    }
    else {
        unsigned int piDigit = computePiDigit(digitNum);
        m_computedDigits[digitNum] = piDigit;
        return true;
    }
}

void PiComputer::printPi() {
    std::cout << "3.";
    for (unsigned int i = 1; i <= m_numDigits; i++) {
        std::cout << m_computedDigits[i];
    }
    std::cout << std::endl;
}
...
int PiComputer::inv_mod(int x, int y)
{
	int q, u, v, a, c, t;
...
/* return (a^b) mod m */
int PiComputer::pow_mod(int a, int b, int m)
{
	int r, aa;
...
int PiComputer::next_prime(int n)
{
	do {
		n++;
	} while (!is_prime(n));
...
unsigned int PiComputer::computePiDigit(int n)
{
...
//
// ------------------------------------------------------------------
double PiComputer::powneg(unsigned long long b, long long pow)
{
	double r = std::pow(b, -pow);
	return 1.0 / r;
}

unsigned long long PiComputer::s(unsigned long long j, unsigned long long n)
{
...
unsigned long long PiComputer::piDigitHex(unsigned long long n)
{
	const unsigned long long D = 14;
	const unsigned long long M = static_cast<unsigned long long>(std::pow(16, D));
	const unsigned long long SHIFT = 4 * D;
